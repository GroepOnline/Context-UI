...
class TerminalRenderer {
    ...
    //  WORKING SET BOX
    // ================================================================
    renderWorkingSetBox(workingSet) {
        const lines = [];
    ...
    renderTokens(status) {
        ...
        lines.push(this.makeLine(` ${BOLD}Total${RESET}      ${this.formatk(status.used)} / ${this.formatk(status.total)}  ${GRAY}${Math.round(status.fillPercentage)}%${RESET}`));
        lines.push(this.makeLine(` ${BOLD}Remaining${RESET}  ${this.formatk(status.remaining)}`));
        lines.push(this.makeFooter());
        return lines.join('\n');
    }
    // ================================================================
    ...
    truncateVisible(text, maxVisibleChars) {
        if (this.visibleLength(text) <= maxVisibleChars)
            return text;
        const ellipsis = '...';
